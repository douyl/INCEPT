# Code_Training/train/ssl_meta_arch.py
...
from Code_Training.loss import DINOLoss, iBOTPatchLoss, KoLeoLoss
from Code_Training.models import build_model_from_cfg
from Code_Training.layers import DINOHead
from Code_Training.utils.utils import has_batchnorms
from Code_Training.utils.param_groups import get_params_groups_with_decay, fuse_params_groups
from Code_Training.fsdp import get_fsdp_wrapper, ShardedGradScaler, get_fsdp_modules, reshard_fsdp_model

from Code_Training.models.vision_transformer import BlockChunk
...
class SSLMetaArch(nn.Module):
    ...
    def forward_backward(self, inputs_data, teacher_temp):
        ...
        def get_teacher_output():
            ...
            teacher_cls_tokens = teacher_backbone_output_dict["x_norm_clstoken"] # (B*2, C, D)
            teacher_cls_tokens = teacher_cls_tokens.mean(dim=1) # (B*2, C, D) -> (B*2, D)
            teacher_cls_tokens = teacher_cls_tokens.chunk(n_global_crops_teacher) # tuple of (B, D)
        ...
        student_local_cls_tokens = student_local_cls_tokens.mean(dim=1)
        inputs_for_student_head_list.append(student_local_cls_tokens.unsqueeze(0))

        # 1b: global crops cls tokens
        student_global_cls_tokens = student_global_backbone_output_dict["x_norm_clstoken"]
        student_global_cls_tokens = student_global_cls_tokens.mean(dim=1)
