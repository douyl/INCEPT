...
def get_channel_coords():
    # Directly fetch from MNE standard_1020 without mapping as requested
    montage = mne.channels.make_standard_montage('standard_1020')
    pos_dict = montage.get_positions()['ch_pos']
    # Note: If 'T3' etc are missing in standard_1020, this will raise KeyError. 
    # Assumes environment/mne version supports these names or standard aliases are active.
    coords_np = np.stack([pos_dict[name] for name in FACED_CHANNELS_32])
    logger.info(f"Loaded 32 Channel Coordinates from MNE standard_1020")
    return torch.tensor(coords_np, dtype=torch.float32)

# ==========================
# 2. Dataset
# ==========================
class FACEDDataset(Dataset):
    def __init__(self, root, split="train"):
        self.classes = ['Anger', 'Disgust', 'Fear', 'Sadness', 'Neutral', 'Amusement', 'Inspiration', 'Joy', 'Tenderness']
        self.class_to_idx = {cls: i for i, cls in enumerate(self.classes)}
        self.samples = []
        
        # Determine segments (N) from path (e.g. Segment_10s -> 10)
        match = re.search(r'Segment_(\d+)s', root)
        self.n_segments = int(match.group(1)) if match else 1
        

        # Split Logic: Train(0-79), Val(80-99), Test(100-122)
        all_subs = sorted([d for d in os.listdir(root) if d.startswith('sub') and os.path.isdir(os.path.join(root, d))])
        
        if split == "train":
            target_subs = [s for s in all_subs if int(s[3:]) <= 79]
        elif split == "val":
            target_subs = [s for s in all_subs if 80 <= int(s[3:]) <= 99]
        else: # eval/test
            target_subs = [s for s in all_subs if 100 <= int(s[3:]) <= 122]
        ...
        self.ch_idx = torch.arange(32)
        self.t_idx = torch.arange(self.n_segments)
        logger.info(f"[{split.upper()}] Samples: {len(self.samples)} | Classes: 9 | Channels: 32 | Segments: {self.n_segments}")

    def __len__(self): return len(self.samples)

    def __getitem__(self, index):
        path, label = self.samples[index]
        data = np.load(path) # (C, N, T) = (32, N, 250)
        ...
        # To Tensor: (C, N, T) -> (T, C, N) for Model (250, 32, N)
...
def load_backbone(cfg, ckpt_path, n_segments=1):
    ...
    logger.info(f"Monkey-patched {ViT.__name__}.get_eeg_coords for FACED 32ch.")

    args = dict(num_channels=32, num_patches_per_channel=n_segments, patch_time_dim=cfg.dataset.patch_time_dim, 
...
class LPModel(nn.Module):
    def __init__(self, backbone, input_dim, num_classes=9, n_cls_layers=4, use_patch=False):
        super().__init__()
...
def run_linearprobe_seed(cfg, args, dev, current_seed):
    ...
    ds_tr = FACEDDataset(args.data, "train")
    ds_val = FACEDDataset(args.data, "val")
    ds_test = FACEDDataset(args.data, "eval")
    ...
    model = LPModel(backbone, input_dim, num_classes=9, n_cls_layers=args.n_cls_layers, use_patch=args.use_patch).to(dev)
    ...
    save_path = os.path.join(args.output_dir, "FACED", f"temp_lp_ckpt_{run_id}.pth")
    ...
    if best_epoch != -1 and os.path.exists(save_path):
        ...
        global_save_path = os.path.join(args.output_dir, "FACED", f"{run_id}_LP_seed{current_seed}.pth")
...
if __name__ == "__main__":
    ...
    os.makedirs(os.path.join(args.output_dir, "FACED"), exist_ok=True)
