...
class MumtazDataset(Dataset):
    def __init__(self, root, split="train"):
        # H -> 0 (Healthy), MDD -> 1 (Major Depressive Disorder)
        self.classes = ['H', 'MDD']
        self.class_to_idx = {cls: i for i, cls in enumerate(self.classes)}
        self.samples = []
        
        self.n_channels = 19
        self.n_segments = 5   # -> (19, 5, 250)
        ...
        # Traverse: Split Folder -> Class Folder (H/MDD) -> .npy files
...
def run_finetune(cfg, args, dev, current_seed):
    logger.info("\n" + "="*20 + " Mumtaz2016 Finetune (19ch) " + "="*20)
    ...
    ds_tr = MumtazDataset(args.data, "train")
    ds_val = MumtazDataset(args.data, "val")
    ds_test = MumtazDataset(args.data, "eval")
    ...
    save_path = os.path.join(args.output_dir, "Mumtaz2016", f"temp_ckpt_{run_id}.pth")
    ...
    if best_epoch != -1 and os.path.exists(save_path):
        ...
        global_save_path = os.path.join(args.output_dir, "Mumtaz2016", f"{run_id}_FT_seed{current_seed}.pth")
...
if __name__ == "__main__":
    ...
    os.makedirs(os.path.join(args.output_dir, "Mumtaz2016"), exist_ok=True)
