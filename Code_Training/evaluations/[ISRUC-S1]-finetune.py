...
class FTModel(nn.Module):
    ...
    def forward(self, x, c, t):
        ...
        epoch_features = self.head(x_embed)                                # (bz*seq_len, N*768) -> (bz*seq_len, 768)
        ...
        out = self.classifier(seq_features)                                # (bz, seq_len, 5)
...
def run_finetune(cfg, args, dev, current_seed):
    ...
    with torch.no_grad():
        ...
        # Dummy pass requires (1, 250, 6, 30)
        dummy_x = d[0][0].unsqueeze(0).to(dev)
    ...
    model = FTModel(backbone, input_dim, num_classes=5, drop=args.dropout, n_cls_layers=args.n_cls_layers, use_patch=args.use_patch).to(dev)
    
    optimizer = torch.optim.AdamW([{'params': model.backbone.parameters(), 'lr': args.lr_backbone},
                                   {'params': model.head.parameters(), 'lr': args.lr_head},
                                   {'params': model.sequence_encoder.parameters(), 'lr': args.lr_head},
    ...
    save_path = os.path.join(args.output_dir, "ISRUC", f"temp_ckpt_{run_id}.pth")
    ...
    for ep in range(args.epochs):
        ...
        for step, (x, y, c, t) in enumerate(loaders["train"]):
            ...
            with autocast(enabled=args.use_fp16): 
                out = model(x, c, t) # (bz, seq_len=20, freq=250, ch_num=6, epoch_size=30) -> (bz, seq_len, 5)
                loss = crit(out.view(-1, 5), y.view(-1))  # out -> (bz*seq_len, 5), y -> (bz*seq_len)
    ...
    if best_epoch != -1 and os.path.exists(save_path):
        ...
        global_save_path = os.path.join(args.output_dir, "ISRUC", f"{run_id}_FT_seed{current_seed}.pth")
