...
def get_channel_coords():
    # Directly fetch from MNE standard_1020 using the specific 64-channel list
    montage = mne.channels.make_standard_montage('standard_1020')
    pos_dict = montage.get_positions()['ch_pos']
    coords_np = np.stack([pos_dict[name] for name in PhysioNet_CHANNELS_64])        
    logger.info(f"Loaded 64 Channel Coordinates from MNE standard_1020")
...
class PhysioNetDataset(Dataset):
    def __init__(self, root, split="train"):
        ...
        # Subject Splitting Logic (S001 - S109)
        all_subjects = sorted([d for d in os.listdir(root) if d.startswith('S') and os.path.isdir(os.path.join(root, d))])
        
        if split == 'train':
            subject_list = all_subjects[:70]      # 0 - 69
        elif split == 'val':
            subject_list = all_subjects[70:89]    # 70 - 88
        else: # eval/test
            subject_list = all_subjects[89:109]   # 89 - 109

        # Traverse: Subject -> Class Folder -> .npy files
        ...
        logger.info(f"[{split.upper()}] Samples: {len(self.samples)} | Classes: 4 | Channels: {self.n_channels} | Segments: {self.n_segments}")

    def __len__(self): return len(self.samples)

    def __getitem__(self, index):
        path, label = self.samples[index]
        data = np.load(path) # Expected shape: (64, 4, 250)
...
def load_backbone(cfg, ckpt_path, n_segments=1):
    ...
    logger.info(f"Monkey-patched {ViT.__name__}.get_eeg_coords for PhysioNet 64ch.")

    args = dict(num_channels=64, num_patches_per_channel=n_segments, patch_time_dim=cfg.dataset.patch_time_dim, 
...
class LPModel(nn.Module):
    def __init__(self, backbone, input_dim, num_classes=4, n_cls_layers=4, use_patch=False):
        super().__init__()
...
def run_linearprobe_seed(cfg, args, dev, current_seed):
    logger.info("\n" + "="*20 + " PhysioNet Linear Probing (64ch) " + "="*20)
    ...
    ds_tr = PhysioNetDataset(args.data, "train")
    ds_val = PhysioNetDataset(args.data, "val")
    ds_test = PhysioNetDataset(args.data, "eval")
    ...
    # Load backbone with 64 channels
    ...
    model = LPModel(backbone, input_dim, num_classes=4, n_cls_layers=args.n_cls_layers, use_patch=args.use_patch).to(dev)
    ...
    save_path = os.path.join(args.output_dir, "PhysioNet", f"temp_lp_ckpt_{run_id}.pth")
    ...
    if best_epoch != -1 and os.path.exists(save_path):
        ...
        global_save_path = os.path.join(args.output_dir, "PhysioNet", f"{run_id}_LP_seed{current_seed}.pth")
...
if __name__ == "__main__":
    ...
    os.makedirs(os.path.join(args.output_dir, "PhysioNet"), exist_ok=True)
