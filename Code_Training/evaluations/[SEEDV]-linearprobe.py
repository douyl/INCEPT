import os, sys, yaml, torch, argparse, random, gc, logging, warnings, datetime, mne, re, copy
...
class LPModel(nn.Module):
    def __init__(self, backbone, input_dim, num_classes=5, n_cls_layers=4, use_patch=False):
        super().__init__()
...
def run_linearprobe_seed(cfg, args, dev, current_seed):
    logger.info("\n" + "="*20 + " SEEDV Linear Probing (62ch) " + "="*20)
    ...
    model = LPModel(backbone, input_dim, num_classes=5, n_cls_layers=args.n_cls_layers, use_patch=args.use_patch).to(dev)
    ...
    save_path = os.path.join(args.output_dir, "SEEDV", f"temp_lp_ckpt_{run_id}.pth")
    ...
    if best_epoch != -1 and os.path.exists(save_path):
        ...
        global_save_path = os.path.join(args.output_dir, "SEEDV", f"{run_id}_LP_seed{current_seed}.pth")
