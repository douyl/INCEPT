...
class FTModel(nn.Module):
    def __init__(self, backbone, input_dim, num_classes=9, drop=0.5, n_cls_layers=4, use_patch=False):
        super().__init__()
...
def run_finetune(cfg, args, dev, current_seed):
    ...
    model = FTModel(backbone, input_dim, 9, args.dropout, args.n_cls_layers, args.use_patch).to(dev)
    ...
    save_path = os.path.join(args.output_dir, "FACED", f"temp_ckpt_{run_id}.pth")
    ...
    if best_epoch != -1 and os.path.exists(save_path):
        ...
        global_save_path = os.path.join(args.output_dir, "FACED", f"{run_id}_FT_seed{current_seed}.pth")
