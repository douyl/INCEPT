...
class TUABDataset(Dataset):
    def __init__(self, root, split="train"):
        ...
        # Traverse: Split Folder -> Class Folder (normal/abnormal) -> .npy files
...
class FTModel(nn.Module):
    def __init__(self, backbone, input_dim, num_classes=2, drop=0.5, n_cls_layers=4, use_patch=False):
        super().__init__()
...
def run_inference(loader, model, dev, args):
    ...
    with torch.no_grad():
        for x, y, c, t in loader:
            ...
            probs.extend(out_prob[:, 1].cpu().numpy())  # Store prob of abnormal class (Index 1)
    return preds, targets, probs

def run_finetune(cfg, args, dev, current_seed):
    logger.info("\n" + "="*20 + " TUAB Finetune (19ch) " + "="*20)
    ...
    ds_tr = TUABDataset(args.data, "train")
    ds_val = TUABDataset(args.data, "val")
    ds_test = TUABDataset(args.data, "eval")
    ...
    model = FTModel(backbone, input_dim, 2, args.dropout, args.n_cls_layers, args.use_patch).to(dev)
    ...
    save_path = os.path.join(args.output_dir, "TUAB", f"temp_ckpt_{run_id}.pth")
    ...
    for ep in range(args.epochs):
        ...
        preds, targets, probs = run_inference(loaders["val"], model, dev, args)
    ...
    if best_epoch != -1 and os.path.exists(save_path):
        ...
        final_p, final_t, final_prob = run_inference(loaders["test"], model, dev, args)
        final_metrics = compute_metrics(final_t, final_p, final_prob)
        logger.info(f"FINAL TEST RESULTS (Seed: {current_seed}): {final_metrics}")
        
        global_save_path = os.path.join(args.output_dir, "TUAB", f"{run_id}_FT_seed{current_seed}.pth")
...
if __name__ == "__main__":
    ...
    os.makedirs(os.path.join(args.output_dir, "TUAB"), exist_ok=True)
