...
def run_linearprobe_seed(cfg, args, dev, current_seed):
    logger.info("\n" + "=" * 20 + " TUAR Linear Probing (19ch, 4-class) " + "=" * 20)
    ...
    save_path = os.path.join(args.output_dir, "TUAR", f"temp_lp_ckpt_{run_id}.pth")
    ...
    if best_epoch != -1 and os.path.exists(save_path):
        ...
        global_save_path = os.path.join(args.output_dir, "TUAR", f"{run_id}_LP_seed{current_seed}.pth")
        os.replace(save_path, global_save_path)
    else:
        logger.warning(f"No valid checkpoint saved for Seed {current_seed}, eval skipped.")

    del model, optimizer, scheduler, backbone
    gc.collect()
