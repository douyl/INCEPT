import os, sys, yaml, torch, argparse, random, gc, logging, warnings, datetime, copy
...
def run_linearprobe_seed(cfg, args, dev, current_seed):
    logger.info("\n" + "="*20 + " Mumtaz2016 Linear Probing (19ch) " + "="*20)
    ...
    if len(ds_tr) == 0:
        logger.error("Training dataset is empty. Check data path.")
        return None
    ...
    save_path = os.path.join(args.output_dir, "Mumtaz2016", f"temp_lp_ckpt_{run_id}.pth")
    ...
    if best_epoch != -1 and os.path.exists(save_path):
        ...
        global_save_path = os.path.join(args.output_dir, "Mumtaz2016", f"{run_id}_LP_seed{current_seed}.pth")
